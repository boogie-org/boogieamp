 ...
 * 
 */
public class AltControlFlowFactory extends AbstractControlFlowFactory {

	
	public AltControlFlowFactory(Unit astroot, TypeChecker tc) {		
		super(astroot, tc);				
	}
	...
	protected void constructCfg(ProcedureDeclaration proc, CfgProcedure cfg) {		
		...
		if (proc.getBody() != null) {
			...
			//if we use alternative return encoding, we
			//need a local helper variable that indicates if the function
			//has returned.
			context.localVars.put("$returned", new CfgVariable("$returned", BoogieType.boolType, false, false, false, false));
			...
			for (VariableDeclaration vdecl : body.getLocalVars()) {
				vars = varList2CfgVariables(vdecl.getVariables(), false, false,
						false, false);
			...
			//if we use alternative return encoding, we
			//need to initialize the local helper variable to false
			CfgStatement init = new CfgAssignStatement(proc.getLocation(), 
					new CfgIdentifierExpression(
							proc.getLocation(), 
							context.localVars.get("$returned")), 
						new CfgBooleanLiteral(proc.getLocation(), BoogieType.boolType, false));
			root.addStatement(init, true);
	...
	private BasicBlock constructCfg(Statement[] seq, BasicBlock b) {
		HashSet<BasicBlock> tmp_marked = new HashSet<BasicBlock>();
		BasicBlock nextblock;
		for (int i = 0; i < seq.length; i++) {
			nextblock = constructCfg(seq[i], b);
			if (nextblock != null && !tmp_marked.contains(nextblock)) {
				//TODO: maybe this should be done somewhere else? Testing needed!
				//if we use alternative return encoding we have to 
				//assume for each block that $return is false.
				tmp_marked.add(nextblock);

				nextblock.addStatement(new CfgAssumeStatement(nextblock.getLocationTag(),
						new CfgBinaryExpression(nextblock.getLocationTag(), BoogieType.boolType, BinaryOperator.COMPNEQ, 
								new CfgIdentifierExpression(nextblock.getLocationTag(), context.localVars.get("$returned")), 
								new CfgBooleanLiteral(nextblock.getLocationTag(), BoogieType.boolType, true))
						)
				, true);
			}
			b = nextblock;
	...
	private BasicBlock constructCfg(Statement s, BasicBlock b) {
		...
		if (s instanceof AssertStatement) {
			b.addStatement(new CfgAssertStatement(
					s.getLocation(),
			...
			b.addStatement(new CfgAssignStatement(assign.getLocation(), lhs,
					rhs));
			...
			b.addStatement(new CfgAssumeStatement(
					s.getLocation(),
			...
			b.addStatement(new CfgCallStatement(s.getLocation(), lhs, callee,
					args));
			return b;
			...
			b.returns = true;
			CfgStatement ret = new CfgAssignStatement(b.getLocationTag(), 
					new CfgIdentifierExpression(
							b.getLocationTag(), 
							context.localVars.get("$returned")), 
						new CfgBooleanLiteral(b.getLocationTag(), BoogieType.boolType, true));
			b.addStatement(ret, false);
			BasicBlock nextblock = new BasicBlock(s.getLocation(), b.getLabel()+"$return_dummy");
			b.connectToSuccessor(nextblock);
			return nextblock;
